...
void rb_ExInfo_Free(void* data)
{
	if (data != nullptr)
	{
		FMOD_CREATESOUNDEXINFO* exinfo = reinterpret_cast<FMOD_CREATESOUNDEXINFO*>(data);
		delete exinfo;
	}
}
		...
		if (argc > 1)
		{
			...
			if (argc > 2)
			{
				...
				if (argc > 3)
				{
					...
					{
						if (RTEST(argv[4]))
							exinfo->format = static_cast<FMOD_SOUND_FORMAT>(NUM2INT(argv[4]));
						if (argc > 5)
						{
							if (RTEST(argv[5]))
							...
							if (argc > 6)
							{
								...
								{
									if (RTEST(argv[7]))
										exinfo->suggestedsoundtype = static_cast<FMOD_SOUND_TYPE>(NUM2INT(argv[7]));
									if (argc > 8)
									{
										if (RTEST(argv[8]))
										...
										{
											if (RTEST(argv[9]))
												exinfo->channelorder = static_cast<FMOD_CHANNELORDER>(NUM2INT(argv[9]));
											if (argc > 10)
											{
												if (RTEST(argv[10]))
